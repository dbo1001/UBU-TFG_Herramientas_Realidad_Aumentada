...
En esta sección se presentan los conceptos teóricos más importantes del proyecto.
...
La detección de marcadores (Targets) se trata de una técnica de realidad aumentada en que los sistemas utilizan como referencia espacial, para situar elementos que queremos superponer, una figura o imagen concreta que ha sido especificada previamente. Los marcadores mas comunes y simples son los códigos QR, pues se tratan de imágenes diseñadas para que puedan ser reconocidas fácilmente por las cámaras (ver imagen~\ref{fig:Memoria/ejemploMarcadorCasa}).

\imagen{Memoria/ejemploMarcadorCasa}{Ejemplo un marcador sencillo~\cite{AR_company}.}
...
\imagen{Memoria/ejemploMarcadorTigre}{Ejemplo donde una carta de un tigre es un marcador~\cite{AR_company}.}
...
\imagenPeque{Memoria/market}{Ejemplo de un marcador~\cite{linowes_babilinski_2017}.}

\imagenflotante{Memoria/market2}{Ejemplo de marcadores codificados~\cite{linowes_babilinski_2017}.}

\imagen{Memoria/vuforia_recoObjeto}{Vuforia Object Scanner~\cite{vuforia_scanner}.}	
...
\imagen{Memoria/mergecube}{Mergecube~\cite{barker_2019}.} 
...
Los algoritmos de vSLAM, no solo son usados únicamente para la AR, también pueden ser aplicados para robótica, o los coches autónomos no tripulados.
...
\imagen{Memoria/Sensorama}{ Sensorama~\cite{ARhistory}}
...
\imagen{Memoria/EspadaDeDamocles}{La espada de Damocles~\cite{EspadaDamocles}}
...
\imagen{Memoria/ArQuake}{ Imágenes del juego ArQuake, y el dispositivo en el que se ejecutaba~\cite{ArQuake}}
